...
TextureResources* Texture::GetTextureResource(uint32_t num)
{
	return &pTr[num];
}

Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> Texture::GetTexture(uint32_t num)
{
	return pTr[num].pTexture.Get();
}

Microsoft::WRL::ComPtr<ID3D11SamplerState> Texture::GetSampler(uint32_t num)
{
