...
public:
	...
	void Term();
	uint32_t LoadShader(const std::string& vs, const std::string& ps);
	ShaderResources* GetShader(uint32_t num = 0);
private:
	Shader() :cnt(0), CsoSize(0), pCsoBin(nullptr) {};
	~Shader() {};
	uint32_t CreateVS(const std::string& vsStr, uint32_t n);	// 頂点シェーダ作成
	uint32_t CreatePS(const std::string& psStr, uint32_t n);	// ピクセルシェーダ作成
