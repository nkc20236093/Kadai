...
uint32_t Shader::LoadShader(const std::string& vs, const std::string& ps)
{
	pSr.emplace_back();
	CreateVS(vs, cnt);
	CreatePS(ps, cnt);
...
ShaderResources* Shader::GetShader(uint32_t num)
{
	return &pSr[num];
}

// 頂点シェーダ作成
uint32_t Shader::CreateVS(const std::string& vsStr, uint32_t n)
{
...
// ピクセルシェーダ作成
uint32_t Shader::CreatePS(const std::string& psStr, uint32_t n)
{
	// 生成が正常に行われたか判断する為に用意
	HRESULT hr = S_OK;
	// ファイルの拡張子を取得する
...
void  Shader::ReadCSOFile(LPCWSTR szFileName)
{
	...
	if (binfile.is_open()) {
		uint32_t fsize = static_cast<uint32_t>(binfile.seekg(0, std::ios::end).tellg());
